...
class Wallet:
    ...
    def __init__(self):
        ...
        self.payment_signin_key = self.paymentKey()
        self.payment_verification_key = self.verificationKey()

        self.payment_signin_key
        self.payment_verification_key
    ...
    def paymentKey(self):
        ...
        self.payment_key_pair = PaymentKeyPair.generate()
        # payment signin key generation
        self.payment_signin_key = self.payment_key_pair.signing_key
        # saving results in a json file
        self.payment_signin_key.save("payment_key.json")
    ...
    def verificationKey(self):
        ...
        # generating the verification key address
        self.payment_verification_key = self.payment_key_pair.verification_key

        self.payment_verification_key.save("verification_key.json")

        return self.payment_verification_key

    # stake key for users stake accesibility
    def stakeValidators(self):

        # stake key pair generation  per each new user
        self.stake_key_pair = StakeKeyPair.generate()
        # stake key generation
        self.stake_signing_key = self.stake_key_pair.signing_key
        # stake verification
        self.stake_verification_key = self.stake_key_pair.verification_key

        self.stake_verification_key.save("stake_verification_key.json")

        return self.stake_verification_key

    # # payment address
    def address(self, verification_key):  # use the newly generated keys to create a new wallet for each user
...
# interacting with users wallets
class WalletInteraction(object):
    """
